...
class Trainer(object):
    def __init__(self, model, optimizer, dataloader, graph, args, prompt=None, prompt_2=None):
        ...
        if prompt:
            ...
            self.val_loader = dataloader['val_p']
            self.test_loader = dataloader['test']
        ...
        self.best_p_2_path = os.path.join(self.args.log_dir, 'best_p_2_model.pth')
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(self.train_loader)):
            ...
            repr1, repr2 = self.model(data, self.graph, self.prompt, self.prompt_2)  # nvc
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                repr1, repr2 = self.model(data, self.graph, self.prompt, self.prompt_2)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            if val_epoch_loss < best_loss:
                ...
                if self.prompt_2:
                    save_p_2_dict = self.prompt_2.state_dict()
                if not self.args.debug:
                    if self.prompt:
                        self.logger.info('**************Current best P model saved to {}'.format(self.best_p_path))
                        torch.save(save_p_dict, self.best_p_path)
                    if self.prompt_2:
                        self.logger.info('**************Current best P2 model saved to {}'.format(self.best_p_2_path))
                        torch.save(save_p_2_dict, self.best_p_2_path)
        ...
        if self.prompt_2:
            p_2_state_dict = save_p_2_dict if self.args.debug else torch.load(
                self.best_p_2_path, map_location=torch.device(self.args.device))
            self.prompt_2.load_state_dict(p_2_state_dict)
    ...
    @staticmethod
    def test(model, dataloader, scaler, graph, logger, args, prompt=None, prompt_2=None):
        model.eval()
        if prompt:
            prompt.eval()
        if prompt_2:
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(dataloader):
                repr1, repr2 = model(data, graph, prompt, prompt_2)
        ...
        # outflow 
        out_mae, out_mape = test_metrics(y_pred[..., 1], y_true[..., 1])
        logger.info("OUTFLOW, MAE: {:.2f}, MAPE: {:.4f}%".format(out_mae, out_mape * 100))
        test_results.append([out_mae, out_mape])

        logger.info("{:.2f},{:.2f},{:.4f}%,{:.4f}%".format(mae, out_mae, mape * 100, out_mape * 100))
